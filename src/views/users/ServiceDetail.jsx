      ...
      <CCardHeader className="pb-0">
        ...
        <div className="d-flex align-items-start gap-3 mb-3">
          <div style={{
            ...
            background: '#321fdb', display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            {allImages[0]
              ? <img src={allImages[0].url || allImages[0]} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 8 }} onError={(e) => { e.target.style.display = 'none' }} />
              : <span style={{ color: '#fff', fontSize: 24, fontWeight: 700 }}>{service.title?.charAt(0).toUpperCase()}</span>
            }
          </div>
            ...
            <div className="d-flex flex-wrap gap-1">
              ...
              {avgRating && (
                <CBadge color="warning" textColor="dark">
                  <CIcon icon={cilStar} size="sm" className="me-1" />{avgRating}
                </CBadge>
              )}
            </div>
          </div>
          <div className="text-end">
            <div className="fw-bold fs-5 text-primary">{fmtPrice(service.priceMinor, service.priceUnit)}</div>
            ...
            <CRow className="g-3">
              ...
              {service.description && (
                <CCol md={12}>
                  ...
                  <div className="small" style={{ whiteSpace: 'pre-wrap' }}>{service.description}</div>
                </CCol>
              )}
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        cursor: 'pointer', background: '#eee', flexShrink: 0,
                      }}
                    >
                      <img
              ...
              <>
                {avgRating && (
                  <div className="mb-3 d-flex align-items-center gap-2">
                    ...
                    <span className="small text-muted">/ 5 average from {service._count?.reviews} reviews</span>
                  </div>
                )}
                    ...
                    <div key={r.id} className="p-3 bg-body-secondary rounded">
                      <div className="d-flex justify-content-between align-items-center mb-1">
                        <div className="d-flex align-items-center gap-2">
                          <div style={{
                            width: 30, height: 30, borderRadius: '50%', background: '#321fdb',
          ...
          {allImages.length > 1 && (
            <>
              <div
                ...
                style={{ position: 'absolute', left: 24, color: '#fff', fontSize: 36, cursor: 'pointer', userSelect: 'none' }}
              >&#8249;</div>
              <div
                onClick={(e) => { e.stopPropagation(); setLightbox((lightbox + 1) % allImages.length) }}
                style={{ position: 'absolute', right: 24, color: '#fff', fontSize: 36, cursor: 'pointer', userSelect: 'none' }}
              >&#8250;</div>
            </>
          )}
        </div>

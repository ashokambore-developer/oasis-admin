...
const fmtPrice = (minor) =>
  minor == null ? '-' : formatRupees(minor, { decimals: 0 })

const STATUS_COLOR = {
  DRAFT: 'secondary', ACTIVE: 'success', RUNNING: 'info',
  CANCELLED: 'danger', COMPLETED: 'dark', ARCHIVED: 'secondary',
}
const PAYMENT_STATUS_COLOR = {
  PAID: 'success', PENDING: 'warning', CANCELLED: 'secondary',
  REFUNDED: 'info', FAILED: 'danger',
}
const ASSIGNMENT_STATUS_COLOR = {
  PENDING: 'warning', ACCEPTED: 'success', REJECTED: 'danger',
  CANCELLED: 'secondary', COMPLETED: 'dark',
}
const BOOKING_STATUS_COLOR = {
  PENDING: 'warning', SUCCESS: 'success', CANCELLED: 'secondary',
...
const Pill = ({ label, value }) => (
  <div className="text-center px-3" style={{ borderRight: '1px solid var(--cui-border-color)' }}>
    ...
    <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: 1 }} className="text-muted">{label}</div>
  </div>
)
...
const InfoRow = ({ label, value, onClick }) => (
  ...
  >
    ...
    <span className="small fw-semibold text-end" style={{ maxWidth: '60%', wordBreak: 'break-word', color: onClick ? 'var(--cui-link-color)' : undefined }}>{value ?? '-'}</span>
  </CListGroupItem>
)
...
const TripDetail = () => {
    ...
    <div className="d-flex justify-content-end mb-3">
      {!editing ? (
        <CButton size="sm" color="outline-primary" onClick={() => setEditing(true)}>
          <CIcon icon={cilPencil} className="me-1" size="sm" />Edit
        </CButton>
      ) : (
        <div className="d-flex gap-2">
          <CButton size="sm" color="outline-secondary" onClick={handleCancel} disabled={saveMut.isLoading}>
            <CIcon icon={cilX} className="me-1" size="sm" />Cancel
          </CButton>
          <CButton size="sm" color="primary" onClick={handleSave} disabled={saveMut.isLoading}>
            {saveMut.isLoading ? <CSpinner size="sm" className="me-1" /> : <CIcon icon={cilCheck} className="me-1" size="sm" />}
            Save
          </CButton>
        </div>
      )}
  ...
  return (
    ...
    <CCard>
      <CCardHeader className="pb-0">
        ...
        <div className="d-flex align-items-start gap-3 mb-3">
          <div style={{
            ...
            overflow: 'hidden', background: '#321fdb',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            {coverImg
              ? <img src={coverImg} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { e.target.style.display = 'none' }} />
          ...
          <div className="flex-grow-1">
            ...
            <div className="d-flex flex-wrap gap-1">
              ...
              <CBadge color={DIFFICULTY_COLOR[trip.difficulty] || 'secondary'}>{trip.difficulty}</CBadge>
              {trip.featured && <CBadge color="warning" textColor="dark">Featured</CBadge>}
              {trip.isPublic ? <CBadge color="success">Public</CBadge> : <CBadge color="secondary">Private</CBadge>}
            </div>
          </div>
        </div>

        {/* Stats strip */}
        <div className="d-flex flex-wrap gap-0 mb-3 bg-body-secondary rounded p-2" style={{ overflowX: 'auto' }}>
        ...
        <CNav variant="underline-border">
          {TABS.map(t => (
            <CNavItem key={t.key}>
              <CNavLink active={activeTab === t.key} onClick={() => setActiveTab(t.key)} style={{ cursor: 'pointer' }}>
                {t.label}
              </CNavLink>
            </CNavItem>
          ))}
      ...
      <CCardBody>
        {tabError && <CAlert color="danger" className="py-2 small">{tabError}</CAlert>}
        {tabSuccess && <CAlert color="success" className="py-2 small">{tabSuccess}</CAlert>}
        ...
        <CTabContent>
          ...
          <CTabPane visible={activeTab === 'overview'}>
            ...
            <CRow>
              <CCol md={6}>
                <div className="mb-3">
                  <div className="small fw-semibold text-muted mb-1">Title</div>
                  {editing
                    ? <CFormInput size="sm" value={f('title')} onChange={e => setF('title', e.target.value)} />
                    : <div className="small">{trip.title}</div>
                  }
                </div>
                <div className="mb-3">
                  <div className="small fw-semibold text-muted mb-1">Description</div>
                  {editing
                    ? <CFormTextarea rows={4} value={f('description')} onChange={e => setF('description', e.target.value)} style={{ fontSize: 13 }} />
                    : <div className="small" style={{ whiteSpace: 'pre-wrap' }}>{trip.description || '-'}</div>
                  }
                </div>
                <div className="mb-3">
                  <div className="small fw-semibold text-muted mb-1">Special Needs / Accessibility</div>
                  {editing
                    ? <CFormTextarea rows={2} value={f('specialNeeds')} onChange={e => setF('specialNeeds', e.target.value)} style={{ fontSize: 13 }} />
                    : <div className="small">{trip.specialNeeds || '-'}</div>
                  }
                </div>
              </CCol>
              <CCol md={6}>
                <CListGroup flush className="mb-3">
                  <InfoRow
                    label="Destination"
                    value={destPath}
                    onClick={trip.destination?.id ? () => navigate('/destinations/' + trip.destination.id) : undefined}
                  />
                  <InfoRow label="Start Date" value={editing
                    ? <CFormInput size="sm" type="date" value={f('startDate')} onChange={e => setF('startDate', e.target.value)} />
                    : fmtDate(trip.startDate)
                  } />
                  <InfoRow label="End Date" value={editing
                    ? <CFormInput size="sm" type="date" value={f('endDate')} onChange={e => setF('endDate', e.target.value)} />
                    : fmtDate(trip.endDate)
                  } />
                  <InfoRow label="Start Location" value={editing
                    ? <CFormInput size="sm" value={f('startLocation')} onChange={e => setF('startLocation', e.target.value)} />
                    : trip.startLocation
                  } />
                  <InfoRow label="End Location" value={editing
                    ? <CFormInput size="sm" value={f('endLocation')} onChange={e => setF('endLocation', e.target.value)} />
                    : trip.endLocation
                  } />
                  <InfoRow label="Max Slots" value={editing
                    ? <CFormInput size="sm" type="number" value={f('maxSlots')} onChange={e => setF('maxSlots', e.target.value)} />
                    : trip.maxSlots
                  } />
                  <InfoRow label="Difficulty" value={editing
                    ? <CFormSelect size="sm" value={f('difficulty')} onChange={e => setF('difficulty', e.target.value)}>
                        {['EASY', 'MODERATE', 'CHALLENGING'].map(d => <option key={d}>{d}</option>)}
                      </CFormSelect>
                    : <CBadge color={DIFFICULTY_COLOR[trip.difficulty] || 'secondary'}>{trip.difficulty}</CBadge>
                  } />
                  <InfoRow label="Status" value={editing
                    ? <CFormSelect size="sm" value={f('status')} onChange={e => setF('status', e.target.value)}>
                        {['DRAFT', 'ACTIVE', 'RUNNING', 'CANCELLED', 'COMPLETED', 'ARCHIVED'].map(s => <option key={s}>{s}</option>)}
                      </CFormSelect>
                    : <CBadge color={STATUS_COLOR[trip.status] || 'secondary'}>{trip.status}</CBadge>
                  } />
                  {editing ? (
                    <>
                      <InfoRow label="Featured" value={
                        <div className="form-check form-switch mb-0">
                          <input className="form-check-input" type="checkbox" checked={f('featured')} onChange={e => setF('featured', e.target.checked)} />
                        </div>
                      } />
                      <InfoRow label="Public" value={
                        <div className="form-check form-switch mb-0">
                          <input className="form-check-input" type="checkbox" checked={f('isPublic')} onChange={e => setF('isPublic', e.target.checked)} />
                        </div>
                      } />
                    </>
                  ) : (
                  ...
                  {trip.cancellationReason && <InfoRow label="Cancellation Reason" value={trip.cancellationReason} />}
                  <InfoRow label="Created By" value={trip.createdByUser?.name} />
                  {trip.curatedBy && <InfoRow label="Curated By (Agency)" value={trip.curatedBy.agencyName || trip.curatedBy.user?.name} />}
                ...
                <div className="mb-3">
                  ...
                  {editing
                    ? <CFormInput size="sm" value={f('tags')} onChange={e => setF('tags', e.target.value)} placeholder="comma-separated" />
                    : trip.tags?.length > 0
                      ? <div className="d-flex flex-wrap gap-1">{trip.tags.map(t => <CBadge key={t} color="light" textColor="dark" className="border">{t}</CBadge>)}</div>
            ...
            <CRow className="mt-2">
              <CCol md={6}>
                ...
                {editing
                  ? <CFormTextarea rows={4} value={f('includedServices')} onChange={e => setF('includedServices', e.target.value)} placeholder="One per line" style={{ fontSize: 13 }} />
                  : trip.includedServices?.length > 0
                    ? <ul className="small ps-3 mb-0">{trip.includedServices.map((s, i) => <li key={i}>{s}</li>)}</ul>
                    : <span className="small text-muted">-</span>
                }
              </CCol>
              <CCol md={6}>
                ...
                {editing
                  ? <CFormTextarea rows={4} value={f('excludedServices')} onChange={e => setF('excludedServices', e.target.value)} placeholder="One per line" style={{ fontSize: 13 }} />
                  : trip.excludedServices?.length > 0
                    ? <ul className="small ps-3 mb-0">{trip.excludedServices.map((s, i) => <li key={i}>{s}</li>)}</ul>
                    : <span className="small text-muted">-</span>
                }
              </CCol>
            </CRow>
              ...
              <div className="d-flex flex-column gap-3">
                {trip.itinerary.map((day) => (
                  <div key={day.id} className="border rounded">
                    <div className="d-flex align-items-center gap-2 p-3 bg-body-secondary rounded-top" style={{ borderBottom: '1px solid var(--cui-border-color)' }}>
                      <CBadge color="primary" style={{ minWidth: 36 }}>Day {day.day}</CBadge>
                        ...
                        <div className="d-flex flex-column gap-2">
                          {day.activities.map((act) => (
                            <div key={act.id} className="d-flex gap-3 small">
                              <span className="text-muted fw-semibold" style={{ minWidth: 48 }}>{act.time || '-'}</span>
                              <div>
                                <div className="fw-semibold">{act.activity}</div>
                                {act.description && <div className="text-muted">{act.description}</div>}
                                {act.location && <div className="text-muted"><span className="fw-semibold">Location:</span> {act.location}</div>}
                                {act.gearNeeded?.length > 0 && (
                                  <div className="d-flex flex-wrap gap-1 mt-1">
                                    {act.gearNeeded.map((g, i) => <CBadge key={i} color="light" textColor="dark" className="border">{g}</CBadge>)}
                                  </div>
                                )}
                              </div>
                            </div>
                  ...
                  {trip.participants.map((p, i) => {
                    ...
                    return (
                      <CTableRow key={p.id}>
                        ...
                        <CTableDataCell>
                          <CBadge color={BOOKING_STATUS_COLOR[p.status] || 'secondary'}>{p.status}</CBadge>
                        </CTableDataCell>
                        <CTableDataCell className="small">{p.bookingType || '-'}</CTableDataCell>
                        <CTableDataCell>
                          {latestPay
                            ? <CBadge color={PAYMENT_STATUS_COLOR[latestPay.status] || 'secondary'}>{latestPay.status}</CBadge>
                            : <span className="small text-muted">-</span>
                          }
                        </CTableDataCell>
                        ...
                        <CTableDataCell>
                          {p.user?.id && (
                            <CButton size="sm" color="outline-primary" onClick={() => navigate(`/users/${p.user.id}`)}>
                    ...
                    <CTableRow key={pay.id}>
                      ...
                      <CTableDataCell>
                        <div
                          className="small fw-semibold"
                          style={{ color: 'var(--cui-primary)', cursor: 'pointer' }}
                          onClick={() => pay.fromUser?.id && navigate(`/users/${pay.fromUser.id}`)}
                        >
                          {pay.fromUser?.name || '-'}
                        </div>
                        <div className="small text-muted">{pay.fromUser?.email}</div>
                      </CTableDataCell>
                      <CTableDataCell className="small">{fmtPrice(pay.amountMinor)}</CTableDataCell>
                      <CTableDataCell className="small">{fmtPrice(pay.platformFeeMinor)}</CTableDataCell>
                      <CTableDataCell className="small fw-semibold">{fmtPrice(pay.totalAmountMinor)}</CTableDataCell>
                      <CTableDataCell className="small">{pay.paymentMethod || '-'}</CTableDataCell>
                      <CTableDataCell>
                        <CBadge color={PAYMENT_STATUS_COLOR[pay.status] || 'secondary'}>{pay.status}</CBadge>
                      </CTableDataCell>
                      <CTableDataCell className="small text-muted">{fmtDate(pay.createdAt)}</CTableDataCell>
                      <CTableDataCell>
                        <CButton size="sm" color="outline-primary" onClick={() => navigate(`/payments/${pay.id}`)}>
                          <CIcon icon={cilZoomIn} size="sm" />
                        </CButton>
                      </CTableDataCell>
                    </CTableRow>
                    ...
                    <CTableRow key={a.id}>
                      ...
                      <CTableDataCell>
                        <div
                          className="small fw-semibold"
                          style={{ color: 'var(--cui-primary)', cursor: 'pointer' }}
                          onClick={() => a.providerUser?.id && navigate(`/users/${a.providerUser.id}`)}
                        >
                          {a.providerUser?.name || '-'}
                        </div>
                      ...
                      <CTableDataCell>
                        <div className="small fw-semibold">{a.serviceDetails?.title || a.serviceType}</div>
                      ...
                      <CTableDataCell className="small">{fmtPrice(a.agreedAmountMinor ?? a.totalCostMinor)}</CTableDataCell>
                      <CTableDataCell>
                        <CBadge color={a.paymentStatus === 'PAID' ? 'success' : a.paymentStatus === 'NONE' ? 'secondary' : 'warning'}>
                          {a.paymentStatus}
                        </CBadge>
                      </CTableDataCell>
                      <CTableDataCell>
                        <CButton size="sm" color="outline-primary" title="View details" onClick={() => setViewProvider(a)}>
                          <CIcon icon={cilZoomIn} size="sm" />
                        </CButton>
                      </CTableDataCell>
                    </CTableRow>
              ...
              <>
                {avgRating && (
                  <div className="mb-3 d-flex align-items-center gap-2">
                    <CIcon icon={cilStar} className="text-warning" style={{ width: 20, height: 20 }} />
                    <span className="fs-4 fw-bold text-warning">{avgRating}</span>
                    <span className="small text-muted">/ 5 average from {trip._count?.reviews} reviews</span>
                  </div>
                )}
                <div className="d-flex flex-column gap-3">
                  {trip.reviews.map((r) => (
                    <div key={r.id} className="p-3 bg-body-secondary rounded">
                      <div className="d-flex justify-content-between align-items-center mb-1">
                        <div className="d-flex align-items-center gap-2">
                          <div style={{ width: 30, height: 30, borderRadius: '50%', background: '#321fdb', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
    ...
    <CModal visible={!!viewProvider} onClose={() => setViewProvider(null)} size="lg">
      ...
      <CModalBody>
        {viewProvider && (
          <CRow className="g-3">
            <CCol md={6}>
              <div className="small fw-semibold text-uppercase text-muted mb-2" style={{ letterSpacing: 1 }}>Provider</div>
              <CListGroup flush>
                <CListGroupItem className="d-flex justify-content-between py-2 px-0 border-start-0 border-end-0">
                  <span className="text-muted small">Name</span>
                  <span className="small fw-semibold">{viewProvider.providerUser?.name || '-'}</span>
                </CListGroupItem>
                <CListGroupItem className="d-flex justify-content-between py-2 px-0 border-start-0 border-end-0">
            ...
            <CCol md={6}>
              <div className="small fw-semibold text-uppercase text-muted mb-2" style={{ letterSpacing: 1 }}>Service</div>
              <CListGroup flush>
                <CListGroupItem className="d-flex justify-content-between py-2 px-0 border-start-0 border-end-0">
                  <span className="text-muted small">Title</span>
                  <span className="small fw-semibold">{viewProvider.serviceDetails?.title || viewProvider.serviceType || '-'}</span>
                </CListGroupItem>
                <CListGroupItem className="d-flex justify-content-between py-2 px-0 border-start-0 border-end-0">
                  <span className="text-muted small">Type</span>
                  <span className="small">{viewProvider.serviceDetails?.serviceType || viewProvider.serviceType || '-'}</span>
                </CListGroupItem>
                <CListGroupItem className="d-flex justify-content-between py-2 px-0 border-start-0 border-end-0">
            ...
            <CCol md={6}>
              <div className="small fw-semibold text-uppercase text-muted mb-2" style={{ letterSpacing: 1 }}>Assignment</div>
              <CListGroup flush>
                <CListGroupItem className="d-flex justify-content-between py-2 px-0 border-start-0 border-end-0">
                  <span className="text-muted small">Status</span>
                  <CBadge color={ASSIGNMENT_STATUS_COLOR[viewProvider.assignmentStatus] || 'secondary'}>
                    {viewProvider.assignmentStatus}
                  </CBadge>
                </CListGroupItem>
                <CListGroupItem className="d-flex justify-content-between py-2 px-0 border-start-0 border-end-0">
                  <span className="text-muted small">Payment Status</span>
                  <CBadge color={viewProvider.paymentStatus === 'PAID' ? 'success' : viewProvider.paymentStatus === 'NONE' ? 'secondary' : 'warning'}>
                    {viewProvider.paymentStatus}
                  </CBadge>
                </CListGroupItem>
                <CListGroupItem className="d-flex justify-content-between py-2 px-0 border-start-0 border-end-0">
                  <span className="text-muted small">Agreed Amount</span>
                  <span className="small fw-semibold">{fmtPrice(viewProvider.agreedAmountMinor ?? viewProvider.totalCostMinor)}</span>
                </CListGroupItem>
              </CListGroup>
            </CCol>
            {viewProvider.notes && (
              <CCol md={6}>
                <div className="small fw-semibold text-uppercase text-muted mb-2" style={{ letterSpacing: 1 }}>Notes</div>
                <div className="small bg-body-secondary rounded p-2" style={{ whiteSpace: 'pre-wrap' }}>{viewProvider.notes}</div>
              </CCol>
            )}
          </CRow>
        )}
      ...
      <CModalFooter>
        <CButton color="secondary" onClick={() => setViewProvider(null)}>Close</CButton>
      </CModalFooter>
    </CModal>
    </>
  )
